...
export function InfrastructureTab() {
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      // Query storage.objects directly since the RPC was dropped
      const { data, error } = await supabase
        .from('documents')
        .select('id', { count: 'exact', head: true });
      // Fallback: just show empty if no RPC available
      return [] as { bucket_id: string; file_count: number; total_bytes: number }[];
    },
  });
