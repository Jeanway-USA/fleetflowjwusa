...
const DialogHeader = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn("sticky top-0 z-20 bg-background flex flex-col space-y-1.5 text-center sm:text-left px-4 sm:px-6 pt-4 sm:pt-6 pb-4 border-b border-border rounded-t-lg", className)} {...props} />
);
DialogHeader.displayName = "DialogHeader";

const DialogFooter = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn("sticky bottom-0 z-20 bg-background flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-2 px-4 sm:px-6 pt-4 pb-4 sm:pb-6 border-t border-border rounded-b-lg", className)} {...props} />
);
