...
-- 1. Dimension columns on fleet_loads
...
-- 2. over_dimension_rules table
CREATE TABLE IF NOT EXISTS public.over_dimension_rules (
  id uuid NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  org_id uuid NOT NULL REFERENCES public.organizations(id) ON DELETE CASCADE,
  dimension text NOT NULL CHECK (dimension IN ('height', 'width', 'length')),
  min_inches integer NOT NULL,
  max_inches integer,
  cents_per_mile numeric NOT NULL DEFAULT 0,
...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.over_dimension_rules TO authenticated;
GRANT ALL ON public.over_dimension_rules TO service_role;

ALTER TABLE public.over_dimension_rules ENABLE ROW LEVEL SECURITY;

CREATE POLICY "over_dim_rules_select_same_org"
  ON public.over_dimension_rules FOR SELECT
  TO authenticated
  USING (org_id = public.get_user_org_id(auth.uid()));

CREATE POLICY "over_dim_rules_insert_admin"
  ON public.over_dimension_rules FOR INSERT
  TO authenticated
  WITH CHECK (
    org_id = public.get_user_org_id(auth.uid())
    AND (public.is_owner(auth.uid()) OR public.has_role(auth.uid(), 'dispatcher'::app_role))
  );

CREATE POLICY "over_dim_rules_update_admin"
  ON public.over_dimension_rules FOR UPDATE
  TO authenticated
  USING (
    org_id = public.get_user_org_id(auth.uid())
    AND (public.is_owner(auth.uid()) OR public.has_role(auth.uid(), 'dispatcher'::app_role))
...
CREATE POLICY "over_dim_rules_delete_admin"
  ON public.over_dimension_rules FOR DELETE
  TO authenticated
  USING (
    org_id = public.get_user_org_id(auth.uid())
    AND (public.is_owner(auth.uid()) OR public.has_role(auth.uid(), 'dispatcher'::app_role))
...
CREATE OR REPLACE FUNCTION public.set_over_dimension_rules_org_id()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
DROP TRIGGER IF EXISTS trg_set_over_dimension_rules_org_id ON public.over_dimension_rules;
CREATE TRIGGER trg_set_over_dimension_rules_org_id
  BEFORE INSERT ON public.over_dimension_rules
  FOR EACH ROW EXECUTE FUNCTION public.set_over_dimension_rules_org_id();

DROP TRIGGER IF EXISTS trg_over_dimension_rules_updated_at ON public.over_dimension_rules;
CREATE TRIGGER trg_over_dimension_rules_updated_at
  BEFORE UPDATE ON public.over_dimension_rules
...
SELECT o.id, v.dimension, v.min_inches, v.max_inches, v.cpm, v.sort_order
FROM public.organizations o
CROSS JOIN (VALUES
  ('height', 163, 168, 0.10, 10),
  ('height', 169, 174, 0.20, 20),
  ('height', 175, 180, 0.40, 30),
  ('height', 181, NULL, 0.75, 40),
  ('width',  103, 120, 0.10, 10),
  ('width',  121, 144, 0.20, 20),
  ('width',  145, 168, 0.40, 30),
  ('width',  169, NULL, 0.75, 40),
  ('length', 841, 1020, 0.10, 10),
  ('length', 1021, 1140, 0.20, 20),
  ('length', 1141, 1260, 0.40, 30),
  ('length', 1261, NULL, 0.75, 40)
) AS v(dimension, min_inches, max_inches, cpm, sort_order)
ON CONFLICT (org_id, dimension, min_inches) DO NOTHING;

-- 4. Seed accessorial type "Over-Dimension (Rule 670)" for every org
INSERT INTO public.accessorial_types (org_id, name, default_is_driver_pay, sort_order)
SELECT o.id, 'Over-Dimension (Rule 670)', false, 160
FROM public.organizations o
ON CONFLICT (org_id, name) DO NOTHING;

-- 5. Update onboarding to seed both for new orgs
...
-- 6. Update driver guardrail to block dimension edits
