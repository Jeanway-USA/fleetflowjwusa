...
BEGIN
  ...
  LOOP
    -- Normalize pay type (cpm -> per_mile)
    _norm_pay_type := lower(COALESCE(_driver.pay_type, ''));
    IF _norm_pay_type = 'cpm' THEN _norm_pay_type := 'per_mile'; END IF;

    -- Determine period start: day after last settlement, else first activity, else hire date
    ...
    IF _period_start IS NULL THEN
      SELECT MIN(COALESCE(delivery_date, pickup_date)) INTO _period_start
    ...
    IF _norm_pay_type = 'flat' THEN
      -- Any in-transit or delivered load that overlaps the window counts
      SELECT EXISTS (
        SELECT 1 FROM public.fleet_loads l
        WHERE l.org_id = _org AND l.driver_id = _driver.id
          AND l.status IN ('delivered','in_transit')
          AND COALESCE(l.pickup_date, l.delivery_date) <= _period_end
          AND COALESCE(l.delivery_date, l.pickup_date) >= _period_start
      ) INTO _has_activity;
    ...
    -- Skip if nothing to pay (for flat: no activity; for cpm/pct: no delivered work)
    ...
    -- Recompute YTD across this driver's settlements through period_end
