...
DECLARE
  ...
  _existing public.driver_settlements%ROWTYPE;
  _gross numeric;
  _norm_pay_type text;
...
BEGIN
  ...
  LOOP
    ...
    -- Handle an existing (non-archived) settlement for this driver + period_end
    ...
    IF FOUND THEN
      IF lower(COALESCE(_existing.status, 'draft')) = 'draft' THEN
        DELETE FROM public.driver_settlement_items WHERE settlement_id = _existing.id;
        DELETE FROM public.driver_settlements WHERE id = _existing.id;
      ELSE
        CONTINUE; -- approved/paid: never overwrite
      END IF;
    END IF;
    ...
    ELSE
      ...
      FROM public.driver_settlements
      WHERE org_id = _org AND driver_id = _driver.id AND deleted_at IS NULL;
      IF _ps IS NULL THEN
